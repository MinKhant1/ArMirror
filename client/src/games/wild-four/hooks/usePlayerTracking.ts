...
export function usePlayerTracking() {
  ...
  const processFrame = (
    ...
    sorted.forEach(({ landmarks, faceIndex }, slot) => {
      ...
      lastPacketRef.current.set(slot, { timestamp, centerX, centerY, landmarks, blendshapes, matrix });
    });

    const store = useWildFourStore.getState();
    let players: PlayerSlot[] = [...store.players];

    // Rendering / tracking becomes unusably flickery if face landmarks drop for a single frame.
    // Keep the last good packet for a short grace window so assigned animals & head FX stay stable.
    const PACKET_GRACE_MS = 450;
    const bySlot = new Map<number, (typeof packets)[number]>();
    for (const p of packets) bySlot.set(p.slot, p);
    ...
    for (let slot = 0; slot < MAX_PLAYERS; slot += 1) {
      ...
      if (cached && timestamp - cached.timestamp <= PACKET_GRACE_MS) {
        slotsToUse.set(slot, {
          ...
          matrix: cached.matrix as { data: Float32Array | number[] } | undefined,
        });
      }
    }
